...
- `<name>.schema.json` at the top level — one JSON Schema per domain concept (`document.schema.json`, `chunk.schema.json`, `event.schema.json`, `event-flagged.schema.json`, `answer-overridden.schema.json`, `eval-dataset.schema.json`). `eval-dataset.schema.json` isn't a domain event or record like the others. It is the shape of the retrieval eval set in `eval/dataset.json`, kept here so both tracks can read the same contract.
